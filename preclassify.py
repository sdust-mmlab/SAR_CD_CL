...
def hcluster_new(pix_vec, im_di, cluster_num=5):
    print('... ... 1st round clustering ... ...')
    fcm = FCM(n_clusters=2)
    fcm.fit(pix_vec)
    fcm_lab = fcm.u.argmax(axis=1)
    ...
    for i in range(0, cluster_num):
        idx.append(idx_tmp[idx_sort[i]])
    c = len(idx[cluster_num-1])
    res_lab[idx[cluster_num-1]] = 2

    flag_mid = 0
    for i in range(1, cluster_num):
        c = c+len(idx[cluster_num-1-i])
        if c < ttl:
            res_lab[idx[cluster_num-1-i]] = 2
        elif c >= ttl and c < ttr:
            res_lab[idx[cluster_num-1-i]] = 1.5
            flag_mid = 1
        elif flag_mid == 0:
            res_lab[idx[cluster_num-1-i]] = 1.5
            flag_mid = 1
        else:
            res_lab[idx[cluster_num-1-i]] = 1
    res_lab = res_lab.reshape(ylen, xlen)
    return res_lab
...
def cluster_2types(pix_vec, im_di, change_num=4000, unchange_num=8000):
    print('... ... 1st round clustering ... ...')
    fcm = FCM(n_clusters=2)
    fcm.fit(pix_vec)
    fcm_lab = fcm.u.argmax(axis=1)
    ...
    idxmean = []
    res_lab = np.zeros(ylen * xlen, dtype=np.float32)
    res_lab[:] = 1.5 #表示uncertain

    for i in range(0, 2):
        idx_tmp.append(np.argwhere(fcm_lab==i))
        idxmean.append(im_di.reshape(ylen * xlen, 1)[idx_tmp[i]].mean()) #计算对应聚类的均值

    idx_sort = np.argsort(idxmean) #将中的元素从小到大排列，提取其在排列前对应的index(索引)输出。
    for i in range(0, 2):
        idx.append(idx_tmp[idx_sort[i]])
        #idx中【0】是未变化的；【1】是变化的

    un_random_num = np.random.choice(np.squeeze(idx[0]), unchange_num, replace=False)#idx的shape为【x,1】
    res_lab[un_random_num] = 1.0 #未变化的像素点

    ch_random_num = np.random.choice(np.squeeze(idx[1]), change_num, replace=False)
    res_lab[ch_random_num] = 2.0 #变化的像素点


    ele_num1 = np.sum(res_lab == 1.0)
    ele_num2 = np.sum(res_lab == 2.0)
    print(ele_num1) # 7401 ？？？
    print(ele_num2) # 3468 ？？？
